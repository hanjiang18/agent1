    ...
    while(1)
    {
        ...
        if(this->CheckVisData())
        {
            ...
            for(map<string,VisBundle>::iterator mit = mmVisData.begin();mit!=mmVisData.end();++mit)
            {
                ...
                //cout<<"params::vis::mbShowCovGraph : "<<params::vis::mbShowCovGraph<<endl;
                if(params::vis::mbShowCovGraph)
                    this->PubCovGraph();
                //cout<<"mbShowKFs : "<<params::vis::mbShowKFs<<endl;
                ...
                //cout<<"params::vis::mbShowTraj :"<<params::vis::mbShowTraj<<endl;
                if(params::vis::mbShowTraj)
                    this->PubTrajectories();
                //cout<<"params::vis::mbShowMPs : "<<params::vis::mbShowMPs<<endl;
...
void Viewer::PubLocalMPs()
{
    ...
    if(!Cloud.points.empty())
    {
        ...
        //cout<<"pclMsg.header.stamp : "<<pclMsg.header.stamp<<endl;
...
void Viewer::PubMapPointsAsCloud()
{
    ...
    //cout<<"============="<<mCurVisBundle.mmpMPs.size()<<endl;
    ...
    //cout<<"clouds: "<<Cloud.points.size()<<endl;
    if(!Cloud.points.empty())
    {
        sensor_msgs::PointCloud2 pclMsg;
        pcl::toROSMsg(Cloud,pclMsg);
        pclMsg.header.frame_id = msCurFrame;
        pclMsg.header.stamp = ros::Time::now();
        //cout<< "mCurVisBundle.mNativeId "<<mCurVisBundle.mNativeId<<endl;
        if(mCurVisBundle.mNativeId == 0){
