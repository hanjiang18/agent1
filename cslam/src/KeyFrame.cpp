...
KeyFrame::KeyFrame(Frame &F, mapptr pMap, dbptr pKFDB, commptr pComm, eSystemState SysState, size_t UniqueId)
    : mFrameId(F.mId),mUniqueId(UniqueId),mTimeStamp(F.mTimeStamp),mVisId(-1),
      ...
      fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy),mvuRight(F.mvuRight), mvDepth(F.mvDepth),
      N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn), mDescriptors(F.mDescriptors.clone()),mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth),
...
void KeyFrame::SendMe()
{
    ...
    //cout<<"-----------------------"<<this->mId.first<<"  "<<mId.second<<"  "<<mspComm.size()<<endl;
...
void KeyFrame::ConvertToMessage(ccmslam_msgs::Map &msgMap, g2o::Sim3 mg2oS_wcurmap_wclientmap, kfptr pRefKf, bool bForceUpdateMsg)
{
    ...
    if((mbSendFull || mSysState == eSystemState::SERVER) && !bForceUpdateMsg)
    {
        ...
        for(int i=0;i<mvKeysUn.size();++i){
             ccmslam_msgs::right MsgRight;
             MsgRight.mvuRight=mvuRight[i];
             Msg.mright.push_back(MsgRight);
        }

        for(int idx=0;idx<mvKeysUn.size();++idx) 
            Msg.mvKeysUn.push_back(Converter::toCvKeyPointMsg(mvKeysUn[idx]));
        ...
        for(int i=0;i<passDepth.size();i++){
            ccmslam_msgs::PC MsgPC;
            MsgPC.mDepth=passDepth[i];
            MsgPC.mx=passx[i];
            MsgPC.my=passy[i];
            MsgPC.mblue=passblue[i];
            MsgPC.mgreen=passgreen[i];
            MsgPC.mred=passred[i];
            Msg.mPointCloud.push_back(MsgPC);
...
void KeyFrame::getPCinfo(cv::Mat& color, cv::Mat& depth){
