...
MapPoint::MapPoint(const cv::Mat &Pos, kfptr pRefKF, mapptr pMap, size_t ClientId, commptr pComm, eSystemState SysState, int UniqueId)
...
MapPoint::MapPoint(const cv::Mat &Pos, frameptr pFrame, mapptr pMap, int ClientId, commptr pComm, eSystemState SysState, int UniqueId)
: mFirstKfId(pair<size_t,size_t>(-1,1)), mFirstFrame(pFrame->mId), mUniqueId(UniqueId),
      nObs(0), mTrackReferenceForFrame(defpair),mLastFrameSeen(defpair),
      mpReplaced(nullptr), mfMinDistance(0), mfMaxDistance(0),
      mpRefKF(static_cast<kfptr>(NULL)), mnVisible(1), mnFound(1), mbBad(false),
      mpMap(pMap),
      mbIsEmpty(false), mbPoseLock(false),mbPoseChanged(false), mbSentOnce(false),mbInOutBuffer(false),
      mLoopPointForKF_LC(defpair), mCorrectedByKF_LC(defpair),mCorrectedReference_LC(-1),
      mLoopPointForKF_MM(defpair), mCorrectedByKF_MM(defpair),mCorrectedReference_MM(-1),
      ...
      mMaxObsKFId(0),mbSendFull(true)
{
    ...
    const int level = pFrame->mvKeysUn[UniqueId].octave;
    ...
    */
    ...
    pFrame->mDescriptors.row(UniqueId).copyTo(mDescriptor);
